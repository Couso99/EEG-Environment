...
class OpenBCIBoard(Process):
    ...
    def __init__(self, app, queue, queue_even, queue_odd, streaming, isconnected, log, port=None, baud=115200, filter_data=True, timeout=1):
        ...
        if not self.port:
            self.port = OpenBCIBoard.find_ports()[0] if OpenBCIBoard.find_ports() else False
    ...
    def connect(self):
        ...
        self.log.update_text(str(self.ser.write(b'v')))  # Actualiza el texto del logger indicando el byte V enviado al puerto = indica a la tarjeta que realice un reseteo

        time.sleep(1.2)  # Pausa de 1 segundo
    ...
    def disconnect(self):
        ...
        if self.ser.isOpen():
            self.ser.close()
            self.log.update_text('Serial closed')  # Se actualiza el texto del logger indicando que se ha finalizado la conexión
    ...
    @staticmethod
    def find_ports(active_ports=False, baudrate_=115200, timeout_=1):
        ...
        for port in ports:
            ...
            if active_ports and port in active_ports:
                continue
            try:
                s = serial.Serial(port=port, baudrate=baudrate_, timeout=timeout_)
        ...
        if not openbci_ports:
            print('Cannot find any OpenBCI ports. Try again!!')
            return []            #Retorna un false
        else:
            return openbci_ports     #Retorna el puerto detectado

    @staticmethod
    def get_channel_from_port(port, baudrate=115200, timeout=1):
        ...
        # Si line esta vacía, line[-N] retorna cadena vacía, no IndexError
        channel = line[-6:-4]
        return int(channel)
    ...
    def get_system_status(self):
        self.ser.write(bytearray([0xF0, 0x07]))
        #self.ser.write(b'0x07')
        time.sleep(0.1)
        line = self.print_incoming_text()
        # Si en la cadena se encuentra el mensaje "Success: , significa que el puerto es correcto
        if "Success" in line:
            return True

        return False

    # Obtiene el canal en el que estan dongle y Cython
    def get_channel(self):
        # Para comunicarse con el dongle hay que mandar todos los bytes juntos en un bytearray
        self.ser.write(bytearray([0xF0, 0x00]))
        time.sleep(0.1)
        line = self.print_incoming_text()
        # Si line esta vacía, line[-N] retorna cadena vacía, no IndexError
        channel = line[-6:-4]
    ...
    # Establece el canal del dongle y Cython; 1 a 25
    def set_channel(self, radio_channel):
        self.ser.write(bytearray([0xF0,0x01,radio_channel]))
        time.sleep(1)
        print("Channel set succesfully to channel", radio_channel)
        self.log.update_text(f'Channel set to {radio_channel}')

    # Establece el canal del dongle; 1 a 25
    def set_channel_override(self, radio_channel):
        self.ser.write(bytearray([0xF0,0x02,radio_channel]))
        time.sleep(0.1)
        self.print_incoming_text()
        print("Channel override set succesfully to channel", radio_channel)
        #self.log.update_text(f'Channel set to {radio_channel}')
    ...
    def scan_channels(self):
        if self.get_system_status():
            channel = self.get_channel()
            print("Already connected to channel", channel)
            self.log.update_text(f'Already connected to channel {channel}')
            return channel
        # It must first do a scan trough the whole range of channels in case the Cython board has not been
        #connected to a dongle since power on.
        for i in range(1,26):
            self.set_channel_override(i)
            self.get_system_status()
            self.set_channel_override(i)
            if self.get_system_status():
                self.print_incoming_text()
                print("Successfully connected to channel", i)
                self.log.update_text(f'Connected to {i}')
                return i

        self.log.update_text("There was no Cython board available")
        return 0
